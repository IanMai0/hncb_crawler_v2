...
# 自動上傳 S3
# =========================
def upload_to_s3(local_path: str, category: str) -> bool:
    bucket = os.getenv("AWS_S3_BUCKET", "hnbc-common-crawlerinfo-test")
    ...
    s3_key = f"history/{category}/{today}/{filename}"
...
def download_and_convert_to_csv(url: str, category_name: str, output_dir: str = DEFAULT_OUTPUT_DIR) -> Optional[Tuple[str, str]]:
    ...
    try:
        ...
        # =========================
        try:
            success = upload_to_s3(csv_path, category_name)

            if not success:
                logger.warning(f"⚠️ S3 upload failed but pipeline continues: {csv_path}")
