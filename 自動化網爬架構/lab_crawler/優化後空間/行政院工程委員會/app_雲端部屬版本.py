...
# 自動批次上傳 S3
# =========================
def upload_to_s3(file_paths: List[str]) -> None:
    """
    批次上傳 CSV 到 S3
    """

    bucket = "hnbc-common-crawlerinfo-test"
    prefix = "history/pcc_優良拒往_行政院工程委員會"
    ...
    for local_path in file_paths:
        ...
        try:
            ...
            logger.info(f"☁️ S3 upload success: s3://{bucket}/{s3_key}")

        except Exception as e:
            logger.error(f"❌ S3 upload failed: {local_path} | error={e}")

# ===============
# 自動 logs 上傳 S3
# ===============
def upload_log_to_s3(log_file_path: str) -> None:
    if not os.path.exists(log_file_path):
        logger.error(f"❌ Log 檔不存在: {log_file_path}")
        return
    ...
    try:
        ...
        s3_key = f"pcc/{filename.replace('.log', '')}_{ts}.log"

        bucket = "hnbc-common-crawlerinfo-test"

        s3.upload_file(log_file_path, bucket, s3_key)

        logger.info(f"☁️ Log 上傳成功: s3://{bucket}/{s3_key}")

    except Exception as e:
        logger.error(f"❌ Log 上傳失敗: {e}")
...
def count_csv_rows(path: str) -> Tuple[int, int]:
    ...
    logger.info(f"CSV 總列數(含表頭): {total}, CSV 資料列數(不含表頭): {data_rows}")
...
def download_and_convert_to_csv(url: str, category_name: str, output_dir: str = DEFAULT_OUTPUT_DIR) -> Optional[Tuple[str, str]]:
    ...
    try:
        ...
        logger.info(f"🔄 轉換中：Excel -> CSV -> ✅ 轉換成功!")
...
# 自動異常判斷_csv檔案大小
# =========================
def log_csv_file_info(file_path: str) -> None:
    """
    記錄 CSV 檔案大小與基本資訊
    """
    if not os.path.exists(file_path):
        logger.error(f"❌ CSV 檔案不存在: {file_path}")
        return
    ...
    try:
        ...
        logger.info(
            # f"📊 CSV 檔案資訊 | path={file_path} | "
            f"📊 CSV 檔案資訊:size={file_size_bytes} bytes, {file_size_mb:.2f} MB)"
        )
        ...
        if file_size_bytes < 100:
            logger.warning(f"⚠️ CSV 檔案過小，可能為空或抓取失敗: {file_path}")

    except Exception as e:
        logger.error(f"❌ 無法取得檔案大小: {file_path} | error={e}")
    ...
    with conn.cursor() as cursor:
        ...
        LEFT JOIN (
            ...
            FROM (
                SELECT t.*,
                       ROW_NUMBER() OVER (
                           PARTITION BY Corporation_number
                           ORDER BY Update_Time DESC
                       ) AS rn
                FROM crawlerdb.PccExcellent t
            ) x
            WHERE rn = 1
        ) t
        ...
        WHERE
            -- 新統編（完全不存在）
            t.Corporation_number IS NULL

            OR

            -- 舊統編，但內容有變（排除 Update_Time）
            NOT (
...
def sync_pcc_expire_to_main(conn) -> int:
    ...
    with conn.cursor() as cursor:
        ...
        changed_count = int(cursor.fetchone()["cnt"] or 0)
        logger.info(f"PCC Expire 本日異動數: {changed_count}")
...
def main():
    ...
    log_csv_file_info(excellent_csv)
    log_csv_file_info(blacklist_csv)

    # 2. 自動批次上 S3, 目的地 path："s3://hnbc-common-crawlerinfo-test/history/pcc_優良拒往_行政院工程委員會/"
    upload_to_s3([
        excellent_csv,
        blacklist_csv,
    ...
    # 3. data to DB
    # (不針對數據做額外比對, 在業管明確應用場景出來前, 直接無限寫 DB (tmp table) 當 raw data table)
    ...
    try:
        ...
        # 4. 自動 logs 上 S3, 目的地 path："s3://hnbc-common-crawlerinfo-test/history/pcc_優良拒往_行政院工程委員會/"
        try:
            upload_log_to_s3(LOG_FILE)
        except Exception as e:
