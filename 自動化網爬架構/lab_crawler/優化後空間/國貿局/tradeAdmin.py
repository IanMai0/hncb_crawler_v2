...
# 執行抓取 國貿局基本資訊
def run_crawler_tradeAdmin_basicData(path: str, out_b: str):
...
# 執行抓取 國貿局進口實績
def run_crawler_tradeAdmin_gradeData(path: str, result: str):
    # === 爬蟲前 前置作業 ===
    df = pd.read_csv(path, dtype=str)   # 讀取目標統一編號檔案
    target_ids = df["統一編號"].tolist()  # 定義目標

    # 檢查已完成名單, 避免重複處裡
    try:
        df_result = pd.read_csv(result, dtype=str)
        done_ids = set(df_result["統一編號"].tolist())
    except FileNotFoundError:
        done_ids = set()  # 假設沒有任何已爬取過的統一編號, 就按照默認進行

    print(f"總目標: {len(target_ids)}, 已完成: {len(done_ids)}, <UNK>: {len(target_ids) - len(done_ids)}")
