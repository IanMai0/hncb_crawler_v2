...
def merge_diff_tmp_to_main_taxinfo(conn, logger: Optional[logging.Logger] = None) -> Dict[str, int]:
    ...
    insert_sql = """
        ...
        SELECT
          tmp.Party_ID, tmp.Party_Addr, tmp.Parent_Party_ID, tmp.Party_Name, tmp.PaidIn_Capital, tmp.Setup_Date,
          tmp.Party_Type, tmp.Use_Invoice, tmp.Ind_Code, tmp.Ind_Name, tmp.Ind_Code1, tmp.Ind_Name1,
          tmp.Ind_Code2, tmp.Ind_Name2, tmp.Ind_Code3, tmp.Ind_Name3
        FROM tmp_latest tmp
        LEFT JOIN latest_taxinfo latest
          ON latest.Party_ID = tmp.Party_ID AND latest.rn = 1
        WHERE
          tmp.rn = 1
          AND (
