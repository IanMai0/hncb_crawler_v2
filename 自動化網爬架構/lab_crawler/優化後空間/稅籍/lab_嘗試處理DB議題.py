...
def gcis_Info_csv_to_db(mydb, csv_path: str = DAILY_CSV_NAME):
    ...
    # 2) 穩健讀檔（假設已有read_csv_safely，也可沿用）
    with open(csv_path, newline="", encoding="utf-8-sig") as csvfile:
        rows = list(csv.reader(csvfile))

    # 3) 偵測並只跳過 1 行表頭（若確定 2 行，再改回 2）
    start_idx = 1  # 通常只有一行標題
    # 若你想自動偵測，可檢查 rows[0] 是否包含 "Party_ID" 等關鍵字
    ...
    with mydb.cursor() as cursor:
        for x in range(start_idx, len(rows)):
            ...
            setup_date = date_check_insql(r[5])  # 你的轉換函式
            ...
            vals = (
                ...
                r[2],          # Parent_Party_ID
                r[3],          # Party_Name
                _int_or_none(r[4]),   # PaidIn_Capital
                setup_date,    # Setup_Date (DATE 或 YYYY-MM-DD)
                r[6],          # Party_Type
                r[7],          # Use_Invoice
                r[8],          # Ind_Code
                r[9],          # Ind_Name
...
def new_taxrc(mydb):
    ...
    LEFT JOIN RankedData r
      ON r.Party_ID = a.Party_ID AND r.RowNum = 1
    WHERE
      r.Party_ID IS NULL  -- 全新
      OR NOT (a.Party_Addr       <=> r.Party_Addr)
      OR NOT (a.Parent_Party_ID  <=> r.Parent_Party_ID)
      OR NOT (a.Party_Name       <=> r.Party_Name)
      OR NOT (a.PaidIn_Capital   <=> r.PaidIn_Capital)
      OR NOT (a.Setup_Date       <=> r.Setup_Date)
      OR NOT (a.Party_Type       <=> r.Party_Type)
      OR NOT (a.Use_Invoice      <=> r.Use_Invoice)
      OR NOT (a.Ind_Code         <=> r.Ind_Code)
      OR NOT (a.Ind_Name         <=> r.Ind_Name)
      OR NOT (a.Ind_Code1        <=> r.Ind_Code1)
      OR NOT (a.Ind_Name1        <=> r.Ind_Name1)
      OR NOT (a.Ind_Code2        <=> r.Ind_Code2)
      OR NOT (a.Ind_Name2        <=> r.Ind_Name2)
      OR NOT (a.Ind_Code3        <=> r.Ind_Code3)
      OR NOT (a.Ind_Name3        <=> r.Ind_Name3)
    """
    ...
    rcs_sql = """
      ...
      WHERE NOT EXISTS (
        SELECT 1 FROM crawlerdb.TaxRecord TR
