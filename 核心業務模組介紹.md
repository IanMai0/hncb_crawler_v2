# HNCB Crawler 核心模組技術分析報告
本報告針對專案中五大核心模組（稅籍、國貿、工廠、商工商業、商工公司）進行深度技術拆解。依循「目錄結構」、「程式邏輯」、「三層架構（調度/轉換/資料）」之順序進行分析，並提供優化建議。
---
## 1. 稅籍模組 (Tax)
### 主要目錄
`C:\Users\wits\PycharmProjects\hncb_crawler\自動化網爬架構\lab_crawler\優化後空間\稅籍`
### 主要程式分析
| 程式名稱 | 角色 | 重點描述 |
| :--- | :--- | :--- |
...
### MEMO
*   此模組為專案的 **Gold Standard (黃金標準)**。
*   實現了「Raw Data 優先入庫」與「資料庫層級的差異比對」，資料完整性最高。
### 未來優化建議
1.  **Partitioning**: 若 `TaxInfo` 主表數據量過大，可考慮依 `Setup_Date` 或 `Ind_Code` 進行分區。
2.  **MD5 效能**: 目前 MD5 計算在 SQL 端執行，若 DB 負載過重，可移至 Python 端 `crawler_etl_v3` 計算 Hash 後再入庫。
---
## 2. 國貿局模組 (Trade Bureau)
### 主要目錄
`C:\Users\wits\PycharmProjects\hncb_crawler\自動化網爬架構\lab_crawler\優化後空間\國貿局`
### 主要程式分析
| 程式名稱 | 角色 | 重點描述 |
| :--- | :--- | :--- |
...
## 3. 工廠模組 (Factory)
### 主要目錄
`C:\Users\wits\PycharmProjects\hncb_crawler\自動化網爬架構\lab_crawler\舊版code\factory`
### 主要程式分析
| 程式名稱 | 角色 | 重點描述 |
| :--- | :--- | :--- |
...
### MEMO
*   ETL 邏輯（轉換層）寫得不錯，OOP 結構清晰，特別是針對 PUA 字元的處理很細膩。
*   調度層與資料層是主要弱點。
### 未來優化建議
1.  **重寫調度層**: 廢棄 `lab_flow_control.py`，改用類似 `run_daily_job` 的標準架構。
2.  **實作 DB 入庫**: 補完 `output_data_to_DB`，對接 `crawlerdb`。
3.  **移除絕對路徑**: 將路徑改為相對路徑或配置檔讀取。
---
## 4. GCIS 商工商業模組 (GCIS Business)
### 主要目錄
`C:\Users\wits\PycharmProjects\hncb_crawler\自動化網爬架構\lab_crawler\優化後空間\GCIS`
### 主要程式分析
| 程式名稱 | 角色 | 重點描述 |
| :--- | :--- | :--- |
...
### MEMO
*   公司與商業模組混在一起，導致代碼龐大且難以維護。
*   對於「無資料」或「異常」的判斷邏輯（如 `state` 標記）寫死在爬蟲流程中。
### 未來優化建議
1.  **邏輯分離**: 將公司 (`Company`) 與商業 (`Business`) 的 ETL 類別拆開，因為兩者的欄位定義完全不同。
2.  **異常處理標準化**: 將「疑似商號」、「無資料」等狀態碼標準化，並記錄到 DB 的狀態欄位，而非混在資料欄位中。
